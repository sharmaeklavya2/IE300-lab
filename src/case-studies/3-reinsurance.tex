...
Let us study the Pareto distribution. It has a two positive parameters $z$ and $\alpha$,
...
To do so, we sample 1000 points from $\Pareto(3/2)$ and compute the average.
...
Hence, the central limit theorem does not hold for $\Pareto(3/2)$.

\begin{figure}[!htb]
\centering
\includegraphics{paretoMeanQQE.pdf}
\caption{Estimated QQ plot of 1000 sample averages of 1000 points drawn from $\Pareto(3/2)$
...
\begin{tcolorbox}[mathExe]
\begin{exercise}
Why does the central limit theorem not hold for $\Pareto(3/2)$?
...
\item Create QQ plots of this data (i.e., log of amounts) w.r.t~the exponential distribution
    with parameter 1 and the standard normal distribution.
    \textcolor{progExeSec}{You are not required to draw the reference line.}
...
The discussion of such procedures can be quite evolved and hence is omitted here.
