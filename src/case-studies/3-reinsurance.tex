...
Let $X \sim \mathrm{Pareto}(z, \alpha)$ and $Y = \ln(X/z)$.
...
\item Based on the QQ plots, which distribution does the data (i.e., log of amounts) fit better?
    Justify your answer.
\end{enumerate}
If the log of the amounts follow an exponential distribution,
then the original amounts follow a Pareto distribution (by \cref{exe:pareto-vs-exp}).
% If the log of the amounts follow a normal distribution,
% then the original amounts follow a log-normal distribution.
\end{exercise}
\end{tcolorbox}
...
\item If $X \sim \mathrm{Pareto}(z, \alpha)$ and $\alpha > 1$,
...
a fact that we observed in \cref{exe:real-qq} for \texttt{insurance.csv}.
Hence, we should take advantage of this extra information extracted from the data set.

We will use \cref{eq:prem2} to estimate $\Pi(r)$.
