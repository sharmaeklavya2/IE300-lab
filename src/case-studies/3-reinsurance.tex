...
This case study contains two types of exercises:
\begin{enumerate}
\item Math (blue): \cref{exe:exp:mean-var,exe:exp:y-distr,exe:par:ht,exe:par:ev,exe:pareto-vs-exp,exe:pareto-clt,exe:excess,exe:pihat1-eq-pihat2}.
\item Programming (orange): \cref{exe:exp:y-plot,exe:unif-vs-norm-qq,exe:real-qq,exe:hill,exe:pihat-compare}.
\end{enumerate}

You are required to submit three files to Gradescope:
\begin{enumerate}
\item A \textbf{Jupyter notebook} containing solutions to programming exercises.
    Submit it as a \texttt{.ipynb} file. Do \emph{not} convert it to a \texttt{.pdf} file.
\item A \textbf{math file} containing solutions to math exercises.
    Submit it as a \texttt{.pdf} file. Do \emph{not} submit a \texttt{.docx} file.
\item A \textbf{report}, based on the template provided on Canvas.
    Submit it as a \texttt{.pdf} file. Do \emph{not} submit a \texttt{.docx} file.
\end{enumerate}

All python code must be in the Jupyter notebook.
Do \emph{NOT} put any python code in the math file or the report.

We expect the report to be 2 to 3 pages long,
but there is no penalty for shorter or longer reports.
